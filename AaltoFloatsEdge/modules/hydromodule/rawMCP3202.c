...
int MISO[ADCS]={MISO1, MISO2, MISO3, MISO4}; 

rawSPI_t rawSPI =
{
    .clk     =  10, // GPIO for SPI clock.
    .mosi    =  25, // GPIO for SPI MOSI.
...
int getADCSamples(int argSamples, int argPause)
{
    ...
    */
    ...
    // Allocate memory for array used for storing samples
    uint32_t (*data)[5] = malloc(sizeof(*data) * samples);
    ...
    while (sample<samples)
    {
        ...
        // Loop getting the fresh readings.
        while (now_reading != reading && sample<samples)
        {
            /*
            ...
            */
            ...
            data[sample][0] = (sample+1); // Sample number
    ...
    char filename[128];
    strftime(filename, sizeof(filename), "hydrodata-%d-%m-%YT%H-%M-%S.txt", tm);
    ...
    fprintf(fp, "%d, %.1f, (%.0f/s), samples seconds, rate \n",
        samples, end-start, (float)samples/(end-start));
